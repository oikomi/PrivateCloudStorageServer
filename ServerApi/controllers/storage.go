...
func (this *StorageController) Post() {
	...
	case conf.ACTION_UPLOAD:
		...
		err := this.SaveToFile("file", config.BaseDir + path)
		if err != nil {
			beego.Error(err)
			this.Abort("400")
			return
...
func (this *FileOperation) upload(path string) (*CommResData, error) {
	crd := NewCommResData()
	crd.Status = 1
...
func (this *FileOperation) getDiskUsage() (*DiskUsageData, error) {
	dud := NewDiskUsageData()
	fs := syscall.Statfs_t{}
	var disk DiskStatus
	err := syscall.Statfs(config.BaseDir, &fs)
	if err != nil {
		dud.Status = 1
		return nil, err
	}
